/**
 * Hyperspeed — WebGL 高速公路灯光特效组件
 * 源自 React Bits (https://reactbits.dev/backgrounds/hyperspeed)
 * 配色已改为昆仑工坊主题色
 *
 * NOTE: effectOptions 必须用 useMemo 或常量稳定化引用，避免 useEffect 重跑导致 WebGL 重建
 */
...
/**
 * NOTE: ShaderMaterial 不会触发 onBeforeCompile，因此必须在创建材质前手动替换着色器模板中的 distortion 占位符
 */
function getDistortionCode(distortion: string | Distortion | undefined): string {
  return typeof distortion === 'object' ? distortion.getDistortion : '';
}
...
class CarLights {
  ...
  init() {
    ...
    if (Array.isArray(this.colors)) { colorArray = this.colors.map(c => new THREE.Color(c)); } else { colorArray = [new THREE.Color(this.colors)]; }
    for (let i = 0; i < options.lightPairsPerRoadWay; i++) {
      ...
      aOffset.push(laneX + carWidth / 2, offsetY, offsetZ);
      aMetrics.push(radius, length, spd); aMetrics.push(radius, length, spd);
    ...
    const resolvedVertex = carLightsVertex.replace('#include <getDistortion_vertex>', getDistortionCode(this.options.distortion));
    const material = new THREE.ShaderMaterial({
      fragmentShader: carLightsFragment, vertexShader: resolvedVertex, transparent: true, fog: true,
      uniforms: Object.assign({ uTime: { value: 0 }, uTravelLength: { value: options.length }, uFade: { value: this.fade } }, this.webgl.fogUniforms, (typeof this.options.distortion === 'object' ? this.options.distortion.uniforms : {}) || {})
    });
...
class LightsSticks {
  ...
  init() {
    ...
    let colorArray: THREE.Color[];
    if (Array.isArray(options.colors.sticks)) { colorArray = (options.colors.sticks as number[]).map(c => new THREE.Color(c)); } else { colorArray = [new THREE.Color(options.colors.sticks)]; }
    for (let i = 0; i < totalSticks; i++) {
      const width = random(options.lightStickWidth); const height = random(options.lightStickHeight);
      aOffset.push((i - 1) * stickoffset * 2 + stickoffset * Math.random());
      const color = pickRandom<THREE.Color>(colorArray);
      aColor.push(color.r, color.g, color.b); aMetrics.push(width, height);
    }
    instanced.setAttribute('aOffset', new THREE.InstancedBufferAttribute(new Float32Array(aOffset), 1, false));
    instanced.setAttribute('aColor', new THREE.InstancedBufferAttribute(new Float32Array(aColor), 3, false));
    instanced.setAttribute('aMetrics', new THREE.InstancedBufferAttribute(new Float32Array(aMetrics), 2, false));
    const resolvedVertex = sideSticksVertex.replace('#include <getDistortion_vertex>', getDistortionCode(this.options.distortion));
    const material = new THREE.ShaderMaterial({
      fragmentShader: sideSticksFragment, vertexShader: resolvedVertex, side: THREE.DoubleSide, fog: true,
...
class Road {
  ...
  createPlane(side: number, _width: number, isRoad: boolean) {
    ...
    const resolvedVertex = roadVertex.replace('#include <getDistortion_vertex>', getDistortionCode(this.options.distortion));
    const material = new THREE.ShaderMaterial({
      fragmentShader: isRoad ? roadFragment : islandFragment, vertexShader: resolvedVertex, side: THREE.DoubleSide, fog: true,
...
class App {
  ...
  clock: THREE.Clock; disposed: boolean;
  road: Road; leftCarLights: CarLights; rightCarLights: CarLights; leftSticks: LightsSticks;
  fogUniforms: Record<string, { value: any }>; fovTarget: number; speedUpTarget: number; speedUp: number; timeOffset: number; hasValidSize: boolean;
  ...
  constructor(container: HTMLElement, options: HyperspeedOptions) {
    ...
    this.onWindowResize = this.onWindowResize.bind(this); window.addEventListener('resize', this.onWindowResize);
  ...
  onWindowResize() {
    ...
    this.renderer.setSize(width, height); this.camera.aspect = width / height; this.camera.updateProjectionMatrix(); this.hasValidSize = true;
  }
  init() {
    const options = this.options;
    this.road.init();
  ...
  dispose() {
    this.disposed = true;
    if (this.scene) { this.scene.traverse(object => { const obj = object as unknown as THREE.Mesh; if (!obj.isMesh) return; if (obj.geometry) obj.geometry.dispose(); if (obj.material) { if (Array.isArray(obj.material)) { obj.material.forEach(m => m.dispose()); } else { obj.material.dispose(); } } }); this.scene.clear(); }
  ...
  tick() {
    ...
    if (!this.hasValidSize) { const w = this.container.offsetWidth; const h = this.container.offsetHeight; if (w > 0 && h > 0) { this.renderer.setSize(w, h, false); this.camera.aspect = w / h; this.camera.updateProjectionMatrix(); this.hasValidSize = true; } else { requestAnimationFrame(this.tick); return; } }
    // NOTE: 检测容器尺寸变化
    ...
    if (this.hasValidSize) { const delta = this.clock.getDelta(); this.render(); this.update(delta); }
...
const Hyperspeed: FC<HyperspeedProps> = ({ effectOptions = DEFAULT_EFFECT_OPTIONS }) => {
  ...
  useEffect(() => {
    if (appRef.current) { appRef.current.dispose(); appRef.current = null; const c = hyperspeed.current; if (c) { while (c.firstChild) c.removeChild(c.firstChild); } }
    ...
    const myApp = new App(container, options); appRef.current = myApp;
    myApp.init();

/**
 * LiquidEther — React Bits 真实 GPU 流体模拟背景
 * NOTE: 基于 Three.js WebGL 的完整流体力学模拟
 * 包含 advection、divergence、poisson、pressure 等完整 Navier-Stokes 求解
 * 配色已改为昆仑工坊主题色（碧蓝 #3eede7、铜绿 #5EB8AC、暗宝石蓝 #053154）
 * 源自 https://reactbits.dev/backgrounds/liquid-ether
 */
...
// NOTE: 默认配色使用昆仑工坊赛博主题
const defaultColors = ['#3eede7', '#5EB8AC', '#053154'];
...
export default function LiquidEther({
  ...
  // NOTE: 稳定化 colors 引用 —— 只有颜色值真正变化时才更新
  // 避免父组件重渲染导致 colors 数组引用变化 → useEffect 重跑 → WebGL 上下文销毁重建 → 白屏闪烁
...
`;
    ...
    class Advection extends ShaderPass {
      ...
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      update(...args: any[]) {
        const { dt: d, isBounce: b, BFECC: bf } = (args[0] || {}) as { dt?: number; isBounce?: boolean; BFECC?: boolean };
    ...
    class ExternalForce extends ShaderPass {
      ...
      update(...args: any[]) {
        ...
        uniforms.scale.value.set(cs, cs);
        super.update();
      }
    }
    ...
    class Viscous extends ShaderPass {
      ...
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      update(...args: any[]) {
        const { viscous: v, iterations, dt: d } = (args[0] || {}) as { viscous?: number; iterations?: number; dt?: number };
        if (!this.uniforms) return;
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
    ...
    class Pressure extends ShaderPass {
      ...
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      update(...args: any[]) {
        const { vel, pressure: p } = (args[0] || {}) as { vel?: any; pressure?: any };
  ...
  // NOTE: 初始化 useEffect 只在组件挂载和颜色真正变化时执行
  // 其他参数（mouseForce, autoSpeed 等）全部通过下方第二个 useEffect 热更新，不触发 WebGL 重建

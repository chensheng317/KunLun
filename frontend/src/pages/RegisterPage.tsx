...
export default function RegisterPage() {
  ...
  /**
   * 注册提交
   * NOTE: 当后端返回注册关闭的错误时，弹出友好提示弹窗而非直接显示错误
   */
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (err) {
        // NOTE: 检测是否为注册关闭的特定错误，显示专用弹窗
        if (err.includes('注册') && (err.includes('未开放') || err.includes('关闭'))) {
        ...
        {showClosedModal && (
          ...
          >
            ...
            <motion.div
              ...
              className="relative w-full max-w-sm bg-nexus-surface border border-nexus-border rounded-2xl p-8 shadow-2xl shadow-black/40"
            >
              {/* 顶部辉光 */}
              ...
              {/* 标题 */}
              <h2 className="text-lg font-bold text-nexus-text text-center mb-2">
                注册通道暂未开放
              </h2>

              {/* 说明 */}
              <p className="text-sm text-nexus-muted text-center leading-relaxed mb-6">
                当前平台处于内测阶段，暂不开放公开注册。
                <br />
                如需获取账号，请联系管理员开通。
              </p>
              ...
              <div className="space-y-3">
                <button
                  onClick={() => navigate('/')}
                  className="w-full flex items-center justify-center gap-2 py-3 rounded-xl bg-nexus-primary text-nexus-inverse font-bold text-sm shadow-cyber-glow hover:shadow-cyber-glow-hover hover:bg-nexus-primary/90 transition-all duration-300"
                >
                  <Home size={16} />
                  返回首页
                </button>
                <button
                  ...
                  className="w-full flex items-center justify-center gap-2 py-3 rounded-xl bg-nexus-bg border border-nexus-border text-nexus-muted font-medium text-sm hover:text-nexus-text hover:border-nexus-primary/30 transition-all duration-300"
                >
                  已有账号？去登录
                </button>
              </div>

...
const navItems = [
  ...
  { id: 'workers', label: '数字员工', icon: Cpu, badge: 'FREE' },
  { id: 'factory', label: '数字工厂', icon: Wrench, badge: 'PRO' },
  { id: 'assets', label: '资产库', icon: Database },
...
export default function WorkbenchSidebar({
  ...
  const displayRole = user?.role || '普通用户';
  ...
  return (
    ...
    >
      ...
      <nav className="flex-1 overflow-y-auto overflow-x-hidden py-4 px-2.5 space-y-1.5">
        ...
        <div className="flex items-center justify-between px-2 mb-3">
          ...
          <button
            ...
            className="w-7 h-7 rounded-md flex items-center justify-center text-nexus-muted hover:text-nexus-primary hover:bg-nexus-surface transition-all duration-200 shrink-0"
                ...
                <div className="absolute left-full top-1/2 -translate-y-1/2 ml-2 px-2.5 py-1.5 rounded-lg bg-nexus-surface border border-nexus-border text-xs font-medium text-nexus-text whitespace-nowrap opacity-0 group-hover/nav:opacity-100 pointer-events-none transition-opacity duration-200 z-50 shadow-lg">
                  ...
                  {item.badge && (
                    <span className={`ml-2 text-[10px] px-1 py-0.5 rounded font-bold ${
                      item.badge === 'FREE' ? 'text-nexus-secondary' : 'text-amber-400'
                    }`}>
                      {item.badge}
                    </span>
                  )}
      ...
      <div className={`border-t border-nexus-border transition-all duration-300 ${collapsed ? 'py-3' : 'p-3'}`}>
        <button
          onClick={onUserClick}
          className={`flex items-center transition-all duration-300 group/user relative ${
            collapsed
